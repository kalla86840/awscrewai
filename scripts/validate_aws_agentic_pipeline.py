...
def main():
    ...
    require("RepositoryPolicyText" in cicd_template, "ECR repository policy must be managed by CloudFormation.")
    require("aws ecr set-repository-policy" not in buildspec, "Buildspec must not fail on direct ECR policy repair.")
    ...
    require("Action: lambda:InvokeFunction\n" in endpoint_template, "New Lambda Function URLs must also allow InvokeFunction.")
    require("InvokedViaFunctionUrl: true" in endpoint_template, "InvokeFunction permission must be limited to Function URL calls.")
